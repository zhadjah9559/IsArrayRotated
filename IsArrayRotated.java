/**
 * Created by Vincent on 8/28/2018.
 */
/*==============================================================================
...
public class IsArrayRotated
{
    public static void main(String[] args)
    {
        ...
        rotation = isRotated(original, notRotated);
        if(rotation == true)
        {
    ...
    public static int[] rotateArray(int[] userArray)
    {
        int replace = 0;
        int inc = 1;
        System.out.println("array length= "+userArray.length);
        ...
        else
        {
            ...
            {
                replace = userArray[i];
                userArray[i] = ((userArray.length - inc)+1);
                userArray[userArray.length - inc] = replace;
                inc++;
            }*/
            int num=1;// num to increment last to second to last etc
            for (int i = 0; i <(userArray.length/2) ; i++) {
                replace=userArray[i];// replace = first element
                userArray[i]=userArray[userArray.length-num];// first element becomes last in array and then second to last and so on
                userArray[userArray.length-num]=replace;//last becomes first ,second to last becomes second
                num++;
            }
        }
